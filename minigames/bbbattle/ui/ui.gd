...
const LOG_PATH:String = "user://bbbattle_log.log"
	...
	for card in specific_cards:
		if !card.ability_used:
			match card.stats["Ability Name"]:
				...
				"Leaderboard":
					var dampposty_attack = ceil(float(Saves.items["Dampposty Tokens"]) / 2)
					var dampposty_defense = floor(float(Saves.items["Dampposty Tokens"]) / 2)
					card.add_bonus_attack(dampposty_attack, "Leaderboard")
					card.add_bonus_defense(dampposty_defense, "Leaderboard")
				"The Grind":
