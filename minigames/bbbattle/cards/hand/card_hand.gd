...
func generate_cards():
	...
	for i in range(cards_to_generate.size()):
		...
		card.stats = card.return_stats_from_resource("res://minigames/bbbattle/cards/card_variants/stats/" + cards_to_generate[i] + ".tres")
		card.index = i
...
func add_card_from_resource(number:String, forced_index:int = -1):
	var card = card_scene.instantiate()
	var stats_path = "res://minigames/bbbattle/cards/card_variants/stats/" + number + ".tres"
